...
@userinfobot. Подписку и домен задаёшь в боте, в env их писать не нужно.
...
- ⚙️ **Настройки** — алерты (падение/восстановление, высокий пинг, глобальный
  сбой), время ежедневной сводки, домен.
...
прокси). Внутренний `/sub` остаётся на `127.0.0.1:8081`. Готовый конфиг nginx
бот отдаёт в 🚀 Веб-сервер → 🔧 Конфиг nginx:
